...
if __name__ == '__main__':
    ...
    # 训练集与测试集分类须相等
    if train_label_list != test_label_list:
        assert 'please make sure train labels and test lalels are same!'

    # 生成训练集csv
    train_ptd = PreTrainData(data_dir=train_data_dir, label_list=train_label_list)
    train_ptd.gen_csv_data(csv_path=config['DATASETS']['TRAIN_CSV_FILE'])

    # 生成测试集csv
    test_ptd = PreTrainData(data_dir=test_data_dir, label_list=test_label_list)
